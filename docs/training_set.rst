...
This document describes how to create a training set of configurations using the mbe-automation program.
...
This workflow is more complex and involves three distinct steps: MD sampling, free energy calculation, and phonon sampling. Each step has its own configuration object with several parameters. For detailed information about each parameter, please refer to the source code and docstrings of the ``mbe_automation.configs.training.MDSampling``, ``mbe_automation.configs.quasi_harmonic.FreeEnergy``, and ``mbe_automation.configs.training.PhononSampling`` classes.
