...
2. **External baseline substitution** — replaces the MLIP static cold curve with a 3rd-order polynomial baseline built from user-supplied EOS parameters (`baseline_V0`, `baseline_B0_GPa`, `baseline_B0_prime`). Useful when the MLIP static cold curve should be replaced by a trusted external source, e.g. a high-level DFT or coupled-cluster curve. Can be used with or without reference state forcing.
