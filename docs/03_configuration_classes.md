...
Configuration object for Empirical Electronic Energy Correction (EEC). EEC provides two independent capabilities that can be used separately or together:

1. **Reference state forcing** — anchors a known reference volume ($V_{\text{ref}}$) at reference temperature ($T_{\text{ref}}$). Activated when `reference_state_forcing` is not `"none"`. Three modes modify the electronic energy $E_{\text{el}}(V)$ analytically (`"linear"`, `"inverse_volume"`, `"rigid_shift"`); one mode (`"rebase_to_reference"`) instead rigidly shifts the computed $V_{\text{eos}}(T)$ curve so it passes through $(T_{\text{ref}}, V_{\text{ref}})$, leaving $E_{\text{el}}(V)$ untouched.
2. **External baseline substitution** — replaces the MLIP static cold curve with an external baseline built from user-supplied EOS parameters (`baseline_V0`, `baseline_B0_GPa`, `baseline_B0_prime`). The functional form is controlled by `baseline_curve_type`: either a Birch–Murnaghan EOS (default) or a 3rd-order Taylor polynomial around $V_0$. Useful when the MLIP static cold curve should be replaced by an external source, e.g. a high-level DFT or coupled-cluster curve. Can be used with `reference_state_forcing="none"` or with any of the explicit forcing modes (`"linear"`, `"inverse_volume"`, `"rigid_shift"`). Cannot be combined with `"rebase_to_reference"`, which leaves $E_{\text{el}}(V)$ untouched.

| Parameter                            | Description                                                                                                                                                                                                                                                                                                                                                                                                            | Default Value      |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `reference_state_forcing`            | How the reference state is realised: `"linear"`, `"inverse_volume"`, `"rigid_shift"`, `"rebase_to_reference"`, or `"none"`. `"linear"` applies the correction as $\text{param} \cdot (V - V_{\text{ref}})$. `"inverse_volume"` applies the correction as $\text{param} / V$. `"rigid_shift"` rigidly translates the cold curve along the volume axis by $\Delta V$ such that $\mathrm{d}G/\mathrm{d}V = 0$ at ($V_\text{ref}$, $T_\text{ref}$, $p_\text{ref}$). `"rebase_to_reference"` applies no correction to $E_{\text{el}}(V)$ and instead post-processes the equilibrium-volume curve: $V_{\text{corrected}}(T) = V_{\text{ref}} - V_{\text{eos}}(T_{\text{ref}}) + V_{\text{eos}}(T)$. The first three modes require `equation_of_state="spline"` (which is the default); the rebase mode accepts any supported EOS. The rebase mode is incompatible with `volume_curve="debye"`, with `eos_sampling="pressure"`, and with external baseline substitution. | `"inverse_volume"` |
| `T_ref`                              | Reference temperature (in Kelvin) at which $V_{\text{ref}}$ is enforced. Required if `reference_state_forcing` is not `"none"`. Must exactly match one of the values in the `temperatures_K` array of the `FreeEnergy` configuration.                                                                                                                                                                                 | `None`             |
| `V_ref`                              | Reference volume (in $\text{\AA}^3$ per unit cell of type `cell`) the system should exhibit at $T_{\text{ref}}$. Required if `reference_state_forcing` is not `"none"`. For the explicit forcing modes `V_ref` itself must lie strictly within the sampled volume range. For `"rebase_to_reference"` it is instead the equilibrium volume $V_{\text{eos}}(T_{\text{ref}})$ that must be a valid (converged, non-extrapolated) minimum so the rebase anchor is well-defined. | `None`             |
| `p_ref_GPa`                          | Reference pressure (GPa) used when solving for the `rigid_shift` parameter.                                                                                                                                                                                                                                                                                                                                           | `1.0E-4`           |
| `cell`                               | Unit cell type (`"primitive"` or `"conventional"`) that `V_ref` and the baseline curve parameters correspond to. Volumes are rescaled automatically to match the internal convention of the calculation.                                                                                                                                                                                                                | `"conventional"`   |
| `min_forcing_pressure_GPa`           | Lower bound (GPa) for the EEC equivalent pressure. An error is raised if the required correction falls below this value, guarding against unphysical extrapolations.                                                                                                                                                                                                                                                   | `-5.0`             |
| `max_forcing_pressure_GPa`           | Upper bound (GPa) for the EEC equivalent pressure. An error is raised if the required correction exceeds this value.                                                                                                                                                                                                                                                                                                   | `5.0`              |
| `baseline_V0`                        | Equilibrium volume (in $\text{\AA}^3$ per unit cell of type `cell`) of the external baseline static cold curve. Must be provided together with `baseline_B0_GPa` and `baseline_B0_prime` to activate the external baseline substitution.                                                                                                                                                                               | `None`             |
| `baseline_B0_GPa`                    | Bulk modulus (GPa) of the external baseline static cold curve.                                                                                                                                                                                                                                                                                                                                                         | `None`             |
| `baseline_B0_prime`                  | Pressure derivative of the bulk modulus (dimensionless) of the external baseline static cold curve.                                                                                                                                                                                                                                                                                                                    | `None`             |
| `baseline_E0_kJ_mol_unit_cell`       | Reference energy (kJ/mol per unit cell of type `cell`) of the external baseline static cold curve. If `None`, $E_0$ is taken from the MLIP static cold curve.                                                                                                                                                                                                                                                         | `None`             |
| `baseline_curve_type`                | Functional form of the external baseline curve: `"birch_murnaghan"` (3rd-order Birch–Murnaghan EOS) or `"polynomial"` (3rd-order Taylor expansion around $V_0$). Ignored when no external baseline is specified. Note: for `reference_state_forcing = "rigid_shift"` the polynomial form is always used internally regardless of this setting, because the analytical $\Delta V$ derivation requires polynomial coefficients. | `"birch_murnaghan"` |
