...
1. **Reference state forcing** — adds an empirical term to the crystal electronic energy to enforce a known reference volume ($V_{\text{ref}}$) at reference temperature ($T_{\text{ref}}$). Activated when `reference_state_forcing` is not `"none"`.
...
| Parameter                            | Description                                                                                                                                                                                                                                                                                                                                                                                                            | Default Value      |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `reference_state_forcing`            | Type of empirical electronic energy correction: `"linear"`, `"inverse_volume"`, `"rigid_shift"`, or `"none"`. `"linear"` applies the correction as $\text{param} \cdot (V - V_{\text{ref}})$. `"inverse_volume"` applies the correction as $\text{param} / V$. `"rigid_shift"` rigidly translates the cold curve along the volume axis by $\Delta V$ such that $\mathrm{d}G/\mathrm{d}V = 0$ at ($V_\text{ref}$, $T_\text{ref}$, $p_\text{ref}$). | `"inverse_volume"` |
...
| `V_ref`                              | Reference volume (in $\text{\AA}^3$ per unit cell of type `cell`) the system should exhibit at $T_{\text{ref}}$. Required if `reference_state_forcing` is not `"none"`. Must lie strictly within the sampled volume range; an error is raised otherwise to prevent extrapolation.                                                                                                                                       | `None`             |
