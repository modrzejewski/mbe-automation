...
As noted by the user in the latest commit, the previous concern regarding the safety of the `S_vib` interpolation (when using the Debye curve) is considered a non-issue. This is because the number of valid volumes generated by the workflow remains invariant with respect to temperature. Consequently, if the base EOS sampling points are sufficient, they will be sufficient for all temperatures, mitigating the risk of spline fitting failures.
