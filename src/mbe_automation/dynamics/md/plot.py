...
def trajectory(dataset: str, key: str, save_path: str = None):
    ...
    equilibrium_points = traj.time >= traj.time_equilibration
    ...
    avg_E_total = np.mean(E_total_per_atom[equilibrium_points])
    axes[0].plot(time_ps, E_total_per_atom)
    axes[0].axhline(y=avg_E_total, color="k", linestyle="--", label=f"Average: {avg_E_total:.3f}")
    axes[0].set_ylabel("E_total (eV/atom)")
    axes[0].legend(loc="upper right")

    avg_temp = np.mean(traj.temperature[equilibrium_points])
    axes[1].plot(time_ps, traj.temperature, color="C1")
    ...
    if traj.ensemble == "NPT":
        avg_pressure = np.mean(traj.pressure[equilibrium_points])
        axes[2].plot(time_ps, traj.pressure, color="C2")
        axes[2].axhline(y=traj.target_pressure, color="r", linestyle="--", label=f"Target: {traj.target_pressure:.4f}")
        axes[2].axhline(y=avg_pressure, color="k", linestyle="--", label=f"Average: {avg_pressure:.4f}")
        axes[2].set_ylabel("Pressure (GPa)")
        axes[2].legend(loc="upper right")
        
        avg_volume = np.mean(traj.volume[equilibrium_points])
        axes[3].plot(time_ps, traj.volume, color="C3")
